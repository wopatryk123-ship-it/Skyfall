 ...
 *
 * Everything comes out of the shared procedural PBR library (game.materials)
...
export const WEAPON_MATERIALS = {
  ...
  /**
   * Laminated birch furniture — the AK's handguards, grip and stock.
   *
   * This is the library's own WOOD generator, not a tinted polymer: an AK's
   * furniture is the one part of the gun with real grain in it, the grain runs
   * along the part, and at 60 mm per tile the rings are the right size for a
   * handguard rather than for a floorboard. Orange-brown shellac, low
   * roughness in the polished flats and high in the checkering.
   */
  wood: [
    'wood',
    {
      ...BASE,
      bake: { size: 1024, seed: 640, relief: 0.012 },
      // 0.34 m per tile, not 0.06: the generator lays five plank rows with
      // staggered butt joints per tile, so a fine mapping turns a handguard
      // into brickwork. At this scale one board is wider than the part and
      // what shows is the grain running along it, which is the point.
      scale: 0.34,
      tint: c(0.50, 0.285, 0.135),
      roughness: [0.58, 0.14, 0.22],
      normalStrength: 0.85,
      detail: [16, 0.55, 0.30, 6],
      wear: [0.3, 0.55, 0.45, 0],
      // Handled wood polishes bright at the edges rather than wearing to metal.
      wearColor: 0xa9713a,
      wearMaterial: [0.34, 0.0, 0, 0.6],
      grimeColor: 0x140b05,
      three: { physical: true, specularIntensity: 0.35 },
    },
  ],
...
export class WeaponMaterials {
  /**
   * @param {import('../materials/index.js').MaterialSystem} library the shared
   *        procedural PBR library — the same instance the world is built from,
   *        so a gun and the wall behind it are lit by one set of surfaces.
   */
  constructor(library) {
    this.lib = library;
    this.cache = new Map();
