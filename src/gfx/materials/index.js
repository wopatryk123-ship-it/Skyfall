 ...
 *
 * Public API — reach it with `game.materials`:
 *
...
export class MaterialSystem {
  ...
  /**
   * Tiny Strike drives this directly from `main.js` — the renderer is injected
   * and the first bake happens lazily on the first `get()`, so construction is
   * free and nothing has to be awaited before the world builds.
   *
   * @param {object} opts { renderer, quality: 'low'|'medium'|'high', anisotropy }
   */
  constructor(opts = {}) {
    ...
    this._quality = opts.quality === 'low' ? 0.5 : opts.quality === 'medium' ? 0.75 : 1;
    this._anisotropy = opts.anisotropy ?? 8;
  ...
  get(name, opts = {}) {
    ...
    // Shared and cached for the whole session: whoever asked for it must not
    // dispose it when their own scene graph goes away (see World._disposeLoadedMap).
    mat.userData.owShared = true;
